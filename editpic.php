...
$result = mysql_query("SELECT * FROM photos WHERE user_id='".$_SESSION['SESS_USER_ID'] ."'");
	...
	(<?php 
					...
					$count=mysql_query("SELECT * FROM friends WHERE friends_with='$user_id' AND status='unconf'") or die(mysql_error());
...
$result = mysql_query("SELECT * FROM friends WHERE friends_with='".$_SESSION['SESS_USER_ID'] ."' and status='conf'");
  ...
  <?php
									...
									$user_id=$_SESSION['SESS_USER_ID'];							
								$post = mysql_query("SELECT * FROM friends WHERE friends_with = '$user_id' AND status = 'conf' ")or die(mysql_error());
...
$user= $_SESSION['SESS_FIRST_NAME'];
	...
	if ($error > 0){
			...
			if(!$update=mysql_query("UPDATE users SET profImage = '$location' WHERE user_id='$id'")) {
