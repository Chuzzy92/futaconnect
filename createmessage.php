...
$user_id = $_SESSION['SESS_USER_ID'];
$received = mysql_query("SELECT * FROM messages WHERE receiver = '$user_id'")or die(mysql_error());
...
<div><form name="" method="post">
							...
							<?php
								...
								?>
								<input class="t" type="hidden" name="from" id="inputtype" value="<?php echo $row['user_id']; ?>">
							...
							<?php 
								$user_id=$_SESSION['SESS_USER_ID'];							
								$post = mysql_query("SELECT * FROM friends WHERE friends_with = '$user_id' OR user_id = '$user_id' AND status = 'conf' ")or die(mysql_error());
								...
								$user_id=$_SESSION['SESS_USER_ID'];
									...
									if($myfriend == $user_id){
										...
										$friendsa = mysql_fetch_array($friends);
												...
												echo '<option value = '.$friendsa['user_id'].'> '.$friendsa['FirstName'].' '.$friendsa['LastName'].'</option>';
