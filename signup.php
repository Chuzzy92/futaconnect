...
<?php
	...
	if($address == '') {
     ...
     if($iam == '') {
		$errmsg_arr[] = 'iam field is missing';
		$errflag = true;
	}
	if($ssn == '') {
		$errmsg_arr[] = 'Social security number is missing';
		$errflag = true;
	}
...
//If there are input validations, redirect back to the registration form
	...
	//Create INSERT query
	$qry = "INSERT INTO users(UserName, Password, FirstName, LastName, Address, ContactNo, Url, Birthdate, Gender, profImage, iam, ssn, curcity, month, day, year) VALUES('$login','$password','$fname','$lname','$address','$cnumber','$email','$bday','$gender','$propic','$iam','$ssn','$address','$month','$day','$year')";
	$result = @mysql_query($qry);
	
	//Check whether the query was successful or not
	if($result) {
	$errmsg_arr[] = 'Success You can now log-in to futaconnect';
		$errflag = true;
		if($errflag) {
		$_SESSION['ERRMSG_ARR'] = $errmsg_arr;
		session_write_close();
		...
		session_regenerate_id();
			...
			//$_SESSION['SESS_PRO_PIC'] = $user['profImage'];
			session_write_close();
