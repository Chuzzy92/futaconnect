...
<div class="right">
	...
	<div class="bkb"></div>
			...
			<?php
						...
						$id = $_SESSION['SESS_USER_ID'];
						$post = mysql_query("SELECT * FROM users WHERE user_id != '$id' LIMIT 0,3")or die(mysql_error());
						while($row = mysql_fetch_array($post)){
							echo '
							<ul id="sddm11">
							<li>
								<a href="friendprofile.php?id='.$row['user_id'].'"><img class="img-circle" alt="image" src="'.$row['profImage'].'" height="40" width="40" " />
								<font color="#1d3162">'.$row['FirstName']." ".$row['LastName'].'</font>
								</br>
							
								<a href="addfriend.php?id='.$row['user_id'].'" rel="facebox"style="text-decoration:none;"  >Add as Friend</a></p>
								<hr width=200>
								</ul>
							</li>';
...
<hr align="left" width="420">
	   <?php
echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
$result = mysql_query("SELECT * FROM photos WHERE user_id='".$_SESSION['SESS_USER_ID'] ."'  ORDER BY photo_id DESC LIMIT 0,4");
...
<div class="shoutout">
 ...
 <?php
  ...
  $query  = "SELECT *,UNIX_TIMESTAMP() - date_created AS TimeSpent FROM comment WHERE user_id='".$_SESSION['SESS_USER_ID'] ."' ORDER BY comment_id DESC";
...
while($row = mysql_fetch_assoc($result))
{
   echo '<div class="information">';
	echo '<div class="pic1">';
			$result1 = mysql_query("SELECT * FROM users WHERE user_id='".$_SESSION['SESS_USER_ID'] ."'");
